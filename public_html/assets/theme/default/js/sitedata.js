...
**** VALIDATE WINDOW **
***/ 		
function validateWindow(max_width, max_height)
{
    ...
    var extension = ($('#extension_state').val()=='true') ? 12 : 0;
	...
	// convert frame dims
	var frame_depth_top       = convertToDecimal($("select[name='frame_depth_id'] :selected").val());
	var frame_depth_left      = convertToDecimal($("select[name='frame_depth_id'] :selected").val());
	var frame_depth_bottom    = convertToDecimal($("select[name='frame_depth_id'] :selected").val());
	var frame_depth_right     = convertToDecimal($("select[name='frame_depth_id'] :selected").val());
	...
	var productTypeIndex = $("#product_types_id option:selected").eq(1).val();
    ...
    var side1 = ( A > B ) ? A : B;
    var side2 = ( C > D ) ? C : D;
	var available = acrylicAvailableCheck(side1,side2,max_height,max_width);
	if (available == 'no')
	{
		invalid = true;
		passed = false;
