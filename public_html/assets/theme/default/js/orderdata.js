...
**** VALIDATE WINDOW **
***/ 		
function validateWindow(data, $row)
{
    var $popover = $('.popover-content');
    var product = products_info[$('.popover [name="product_types_id"]').val()];
    var max_width = parseFloat(product.max_width);
    var max_height = parseFloat(product.max_height);
    var frame_depths = [null, .5, .75, 1.5, 2.1];
    var frame_depth = frame_depths[$popover.find('select[name=frame_depth_id]').val()];
	...
	// dimensions
    var A = convertToDecimal($.trim($("#popover-A.measurement").eq(1).val()));
    var B = convertToDecimal($.trim($("#popover-B.measurement").eq(1).val()));
    var C = convertToDecimal($.trim($("#popover-C.measurement").eq(1).val()));
    var D = convertToDecimal($.trim($("#popover-D.measurement").eq(1).val()));
    var E = $("#popover-E.measurement").eq(1).val();
    var F = $("#popover-F.measurement").eq(1).val();

    //var extension = ($($row).find("#extension_state").val() =='true') ? 12 : 0;
    ...
    var laser = $popover.find('[name="freebird_laser"]:checked').length;
    var extension;

    if (window.indow_legacy) {
        if ($popover.find('[name="extension"]:checked').length) {
            if (laser) {
                extension = 11;
            } else {
                extension = 12;
        ...
        if (laser && window.indow_module === 'freebird') {
            A = parseFloat(A) + 1;
            B = parseFloat(B) + 1;
            C = parseFloat(C) + 1;
            D = parseFloat(D) + 1;
        ...
        E = parseFloat(E) + extension;
        F = parseFloat(F) + extension;
    } else if (window.indow_module === 'freebird' && !$popover.find('[name="own_tools"]:checked').length && !window.indow_legacy) {
        E = parseFloat(E) + 2
        F = parseFloat(F) + 2
    }
    A = parseFloat(A);
    ...
    var E_MAX = E; //Math.max(E,E_2,E_3);
    var F_MAX = F; //Math.max(F,F_2,F_3);
	...
	// convert frame dims
	var frame_depth_top       = frame_depth;
	var frame_depth_left      = frame_depth;
	var frame_depth_bottom    = frame_depth;
	var frame_depth_right     = frame_depth;
	var frame_depth_required  = .625; // MAGIC replace with gvar
	var frame_depth_threshold = .876; // MAGIC replace with GVAR
	var frame_step            = $("input[name='frame_step']").val();
    ...
    E = Number(E) + Number(extension);
    F = Number(F) + Number(extension);
	...
	{ 
		...
		alert_notes = alert_notes + '\nInserts installed on narrow frame steps are prone\nto venting. Additional hardware may be required\nto hold the insert in place.\nDiscuss options with the client before ordering.';
	}
	...
	var productTypeIndex = $($row).find("#product_types_id option:selected").val(); // .index() ?
    ...
    //var productTypeAbbrev = product.abbrev;

    // Determine longest sides as basis for availabilty of acrylic
    var side1 = ( A > B ) ? A : B;
    var side2 = ( C > D ) ? C : D;
    var secondary_sizing = product.id == 1 || product.id == 66;
    var available = 'no';
    if (secondary_sizing) {
        available = acrylicAvailableCheck(side1,side2,max_height,max_width) === 'yes' || acrylicAvailableCheck(side1,side2,73.5,97.5) === 'yes' ? 'yes' : 'no';
    } else {
	...
	if (passed == true)
	{
		...
		if (pass_with_warning == true)
		{
			if (validity_check == true)
			{
				...
				alert_notes      = "Window passed validation!\nDon't forget to add any important notes before moving on!" + alert_notes;
			}
			else
			{
		...
		else
		{
			...
			alert_notes =  "Window passed validation! Don’t forget to add any important notes before moving on!" + alert_notes;			
		}
	}
	// POST BACK FORM VALUES FOR INSERT	
	//$("input[name='E']").val(E_MAX);
	//$("input[name='F']").val(F_MAX);
    ...
    $("#validation_response > p").html( '<span class="' + well_type +  '">' + alert_notes + '</span>' );
